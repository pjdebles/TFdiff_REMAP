...
#===============================================================================
#
#         FILE:  buildRankTable.pl
#
#        USAGE:  ./buildRankTable.pl  
#
#  DESCRIPTION:  
#
#      OPTIONS:  ---
...
@ARGV == 2 or die "usage: $0 results_directory file_name \n";
my ($resultsFolder, $resultsFileName) = @ARGV;
...
foreach my $result ( @resultFiles ) {
        processResultFile( $result, \%IDRTableHash );
        }
...
sub processResultFile {
    ...
    while (<INFILE>) {
          ...
          chomp();
          my @fields = split( /\t/ );
          print "TF: $fields[ 0 ]\tline number: $lineNumber\n" if $DEBUG;
          push($IDRTableHash{ $fields[0] }, $lineNumber);
          $lineNumber++;
          }
   close(INFILE);
}
