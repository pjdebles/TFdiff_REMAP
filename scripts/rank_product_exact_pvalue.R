...
if (length(Args) != 1) {
  stop("Usage: ./rank_product_exact_pvalue.R combined_ranks_file (result from 'buildRankTable.pl' script)\n")
}

ranksFile = Args[1]
#for development purpose
#ranksFile = "VEC_genes_Rank_5x.csv"

ranksTable = read.table(ranksFile, row.names=1, sep="\t", stringsAsFactors=FALSE)

####################################################################################
#
# geometricMean - Function to calculate the geometric mean (what did you expect?)
#
...
# example:
#> geometricMean(as.numeric(ranksTable[215,]))
#  [1] 2.620432

####################################################################################
#
# rankProduct - Function to calculate the rank product (what did you expect?)
#

rankProduct <- function(array){
  if(!is.numeric(array)){
    stop("Passed argument must be an array. Consider using sapply for data frames.")
  }
  if(any(array < 0)){
...
rankprodbounds <- function(rho,n,k,Delta){
  ...
  for(j in minj:maxj) {
    ...
    if(j != 0) {
      ...
      d2 <- matrix(rep(log(thisrho),nterms),nrow=nterms,byrow=TRUE) - t(matrix(rep(logn*(k-j+thisparam$b),nrho),nrow=nrho,byrow=TRUE))
...
combinedRankProducts = apply(ranksTable, 1, function(array) rankProduct(array))

n <- dim(ranksTable)[1]
k <- dim(ranksTable)[2]
rankProdStats <- rankprodbounds(combinedRankProducts,n,k,Delta = 'geometric')

#Perform FDR estimation from the collection of p-values.
#Calculate qvalue object:
qobj <- qvalue(rankProdStats)

#Create results table:
resultsTable <- cbind(ranksTable,rankProdStats,qobj$qvalues)
...
baseName = sapply(strsplit(basename(ranksFile),"\\."), function(x) paste(x[1:(length(x)-1)], collapse="."))[1]
resultFile = paste(baseName,"_RP_meta_analysis.csv",sep="")
